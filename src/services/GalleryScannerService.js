  ...
  processProgressData(rawProgress) {
    ...
    switch (stage) {
      ...
      case 'processing_images':
        simpleMessage = `处理照片: ${filesProcessed || 0}/${filesFound || 0} | 失败: ${filesFailed || 0}`;
        break;
    ...
    try {
      ...
      if (onProgress) {
        ...
        const rawProgress = {
          ...
          stageProgress: `目录扫描完成，发现 ${finalFilesFound} 个文件，准备处理照片`,
          scanStartTime: scanStartTime
      ...
      console.log(`Scan analysis: ${deletedUris.length} deleted, ${newUris.length} new files`);
      ...
      if (newImages.length > 0) {
        ...
        console.log(`Processing ${newImages.length} discovered images...`);
        ...
        if (onProgress) {
          ...
          const rawProgress = {
            ...
            stage: 'processing_images',
            stageProgress: `待处理照片: ${newImages.length} 个 | 已处理: 0 个`,
            scanStartTime: scanStartTime
        ...
        // 添加调试：显示每个线程要处理的图片URI
        threadGroups.forEach((group, i) => {
          console.log(`🔍 线程${i+1} 要处理的图片:`, group.map(img => img.fileName));
        });
          ...
          for (let i = 0; i < images.length; i += SAVE_BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (image) => {
              ...
              try {
                ...
                // 在照片信息提取阶段提取EXIF数据（一次读取同时获取时间和位置信息）
                ...
                console.log(`🏷️ 图片 ${image.fileName} 分类结果:`, classification);
            ...
            if (successfulData.length > 0) {
              console.log(`💾 线程${threadIndex} 准备保存 ${successfulData.length} 张图片:`, successfulData.map(img => img.fileName));
              await UnifiedDataService.writeImageDetailedInfo(successfulData, false); // 扫描过程中不更新缓存
              console.log(`✅ 线程${threadIndex} 成功保存 ${successfulData.length} 张图片`);
        ...
        console.log(`Images processing completed: ${processedCount} processed, ${failedCount} failed`);
        ...
        if (onProgress) {
          ...
          const rawProgress = {
            ...
            stage: 'processing_images',
            stageProgress: `照片处理完成: 已处理 ${processedCount} 个，失败 ${failedCount} 个`,
            scanStartTime: scanStartTime
      ...
      if (onProgress) {
        ...
        const rawProgress = {
          ...
          stageProgress: `扫描完成: 发现 ${finalFilesFound} 个文件，处理 ${processedCount} 个照片，移除 ${deletedUris.length} 个文件`,
          scanStartTime: scanStartTime,
