#!/usr/bin/env node
/**
 * 真实版图片智能分类测试 - 对 D:\Pictures 目录下的文件进行分类
 * 使用实际的 ImageClassifierService（需要模型文件）
 */
...
function checkModelFiles() {
  const models = [
    './models/id_card_detection.onnx',
    './models/yolov8s.onnx'
  ];
  
  const missingModels = models.filter(model => !fs.existsSync(model));
  
  if (missingModels.length > 0) {
    console.log('⚠️ 缺少模型文件:');
...
// 动态导入 ImageClassifierService
async function importImageClassifierService() {
  try {
    const { default: ImageClassifierService } = await import('./src/services/ImageClassifierService.js');
    return ImageClassifierService;
  } catch (error) {
    console.error('❌ 导入 ImageClassifierService 失败:', error.message);
    console.log('💡 请检查文件路径和依赖');
    return null;
  }
}

class RealPictureClassificationTester {
  constructor() {
  ...
  async saveResults() {
    ...
    const filename = `real_classification_results_${timestamp}.json`;
  ...
  async runTest() {
    ...
    try {
      ...
      if (!checkModelFiles()) {
        console.log('\n💡 使用模拟版本进行测试...');
        console.log('   运行: node test_simple_classification.js');
        return;
      }
      ...
      // 2. 导入分类器
      console.log('\n📦 导入分类器...');
      const ImageClassifierService = await importImageClassifierService();
      if (!ImageClassifierService) {
        return;
      }
...
// 运行测试
async function main() {
  const tester = new RealPictureClassificationTester();
